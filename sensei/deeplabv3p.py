...
"""
This file is a very minor tweaking of the version published here:
    https://github.com/Golbstein/Keras-segmentation-deeplab-v3.1

Remainder of this string comes from the original file...
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, infer = False,
    ...
    batches_input = Lambda(lambda x: x*2-1)(img_input) # CHANGED BY US: img_input range is in reflectance, (0->1.something)
    ...
    if backbone == 'xception':
        ...
        if input_shape[-1]==3:
            ...
            x = Conv2D(32, (3, 3), strides=(2, 2),
                   name='custom_entry_flow_conv1_1', padding='same')(batches_input)
        ...
        if input_shape[-1]==3: # Use standard RGB-input filters
            x = Conv2D(first_block_filters,
                       kernel_size=3,
                       strides=(2, 2), padding='same',
                       use_bias=False, name='Conv')(batches_input)
        else:                  # Use custom filters, rename as such
            x = Conv2D(first_block_filters,
                       kernel_size=3,
                       strides=(2, 2), padding='same',
                       use_bias=False, name='custom_Conv')(batches_input)

        x = BatchNormalization(
            epsilon=1e-3, momentum=0.9, name='Conv_BN')(x)
    ...
    b4 = BatchNormalization(name='image_pooling_BN', momentum=0.9)(b4)
    ...
    b0 = BatchNormalization(name='aspp0_BN', momentum=0.9)(b0)
    ...
    x = BatchNormalization(name='concat_projection_BN', momentum=0.9)(x)
    x = Activation('relu')(x)
    x = Dropout(0.1)(x)
    ...
    if backbone == 'xception':
        ...
        dec_skip1 = BatchNormalization(
            name='feature_projection0_BN', momentum=0.9)(dec_skip1)
    ...
    if weights == 'pascal_voc':
        if backbone == 'xception':
            ...
            weights_path = get_file('deeplabv3_mobilenetv2_tf_dim_ordering_tf_kernels.h5',
                                    WEIGHTS_PATH_MOBILE,
                                    cache_subdir='models')
        model.load_weights(weights_path, by_name=True) # THEORY: by_name means if layer is renamed (e.g. initial conv, then weights will be random init like last layer.) For now, train with RGB to sanity check
